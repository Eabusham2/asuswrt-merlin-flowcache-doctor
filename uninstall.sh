...
[ -f "$SS" ] && sed -i '/roamctl boot/d; /flowcache-doctor-watchdog/d; /fcd-mlo-runner-heal.sh start/d; /flowcache-doctor-mlo-hw-watchdog/d; /dhd-no-coalesce.sh/d' "$SS"
...
rm -f "$DEST/fcd-lib.sh" "$DEST/fcd-platform-gtbe19000ai.sh" "$DEST/fcd-daemon.sh" \
  ...
  "$DEST/fcd-incident.sh" "$DEST/fcd-wired-incident.sh" "$DEST/fcd-range-desense.sh" \
  "$DEST/dhd-no-coalesce.sh" "$DEST/roamctl" "$DEST/flowcache-doctor.conf" \
