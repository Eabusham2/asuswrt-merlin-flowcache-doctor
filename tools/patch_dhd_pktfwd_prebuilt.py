...
def locate_symbol(data: bytes):
    ...
    (_, e_type, e_machine, _, _, _, e_shoff, _, _, _, _, e_shentsize, e_shnum, e_shstrndx) = hdr
    if e_type != ET_REL:
    ...
    for i in range(e_shnum):
        ...
        sections.append({"name_off": f[0], "type": f[1], "flags": f[2], "addr": f[3], "offset": f[4], "size": f[5], "link": f[6], "info": f[7], "align": f[8], "entsize": f[9]})
    ...
    for pos in range(symtab["offset"], symtab["offset"] + symtab["size"], entsize):
        st_name, st_info, _, st_shndx, st_value, st_size = struct.unpack_from("<IBBHQQ", data, pos)
        if st_name < len(strdata) and cstr(strdata, st_name) == SYMBOL:
...
def read_words(data: bytes, function_file_offset: int):
    return {rel: struct.unpack_from("<I", data, function_file_offset + rel)[0] for rel in ORIGINAL}
...
def main():
    ap = argparse.ArgumentParser(description="Patch/verify GT-BE19000AI prebuilt DHD PKTFWD reassociation invalidation")
    ...
    if args.mode == "patch":
        if before_state == "original":
            ...
            raise SystemExit("Refusing binary patch: dhd_pktfwd_request instruction signature is unknown.\n" + json.dumps(fmt_words(before_words), indent=2))
    ...
    report = {"tool": "FlowCache Doctor DHD PKTFWD prebuilt patcher", "patch_revision": PATCH_REVISION, "file": str(path), "mode": args.mode, "symbol": SYMBOL, "symbol_value": f"0x{after_symbol_value:x}", "symbol_size": after_symbol_size, "function_file_offset": f"0x{after_off:x}", "state_before": before_state, "state_after": after_state, "changed": changed, "sha256_before": before_sha, "sha256_after": after_sha, "semantics": SEMANTICS, "instruction_words_before": fmt_words(before_words), "instruction_words_after": fmt_words(after_words)}
