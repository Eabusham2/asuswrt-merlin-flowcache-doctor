...
6. **Run only the Broadcom/Merlin compile as non-root `docker`.** Broadcom's `prebuild_checks.mk` intentionally rejects root builds. The surrounding GitHub Actions job may stay root so Actions such as `upload-artifact` can write runner command files; explicitly drop to `docker` for `make` and verify `whoami=docker` there.
...
10. **Require the exact normal flash image and distinguish it from the loader image.** For 3006.102.8_4 the normal file is `GT-BE19000AI_3006_102.8_4_emmc_squashfs.pkgtb`; the separate `..._loader.pkgtb` is not the file the normal firmware-upgrade instructions refer to.
11. **Copy and hash the normal firmware immediately after `make` succeeds.** This ensures a later CI/upload failure cannot hide the fact that a valid image was built.
12. **Start a fresh workflow run after changing workflow code.** Do not use `Re-run jobs` from an older run because it can execute the old workflow revision.
13. **Flash only after CI succeeds and the artifact is checked.** Back up router configuration/JFFS first, unzip the GitHub artifact on the Mac, then upload the normal `.pkgtb` through Administration > Firmware Upgrade. Never upload the artifact ZIP itself.
...
Run #5 then completed the full GT-BE19000AI firmware build successfully. Its `BUILD.log` ended with `Done! Image 96813GW has been built in .../targets/96813GW` and showed the exact normal and loader PKGTB files being created there. The workflow nevertheless failed afterward because packaging incorrectly searched `.../image/`. That proved the compile was good and the packaging path was wrong. The permanent workflow now captures the exact normal firmware directly from `targets/96813GW` immediately after a successful build.
