...
BOUNCE_FLUSHES="10 20 60 300 600"  # ladder used instead when a heal re-arms a STILL-RUNNING ladder and the client
                                # is in motion — either the ladder's radio changed (multi-hop bounce) or the heal
                                # is a transition trigger (anything but stale-fdb). Both mean the previous heal
                                # probably landed mid-settle and re-baked poison; field incidents 2026-08-19
                                # 13:47 and 15:35. Shortens only the FIRST rung, so recovery is ~10 s not ~20 s.
                                # "" disables (those heals then use SETTLE_FLUSHES). MIN_GAP floors the drain,
                                # so a first rung below it just waits a pass.
